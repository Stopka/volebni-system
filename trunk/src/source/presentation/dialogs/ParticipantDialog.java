...
public class ParticipantDialog extends AbstractDialog {
    ...
    private void okAction() throws SystemRegException {
        try {
            ...
            if (chooseAct.isEmpty() || firstName.isEmpty() || lastName.isEmpty()
                    || pEmail.isEmpty() || cardID.isEmpty()) {
    ...
    //při editaci přednastaví účastníkovi údaje
    ...
    //z vybraných akcí vytáhne pouze jejich IDčka
    private Collection<Long> getChoosedActionIDs() throws SystemRegException {
        if(chooseAct.isEmpty())
            throw new SystemRegException("Účastník musí mít přiřazenu alespoň jednu akci!");
